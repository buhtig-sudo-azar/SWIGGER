#!/bin/bash
# =============================================================================
# wcd_gun.sh — Пулемёт для тестирования Web Cache Deception v7.0
# =============================================================================
# Назначение:
#   - Двухфазный обстрел: прогрев кэша (жертва) → проверка (атакующий)
#   - Одиночные выстрелы по конкретному разделителю и расширению
...
banner() {
    clear
    echo -e "${CYAN}═══════════════════════════════════════════════════════════════${NC}"
    echo -e "${CYAN}🔫 WCD GUN v7.0 — пулемёт для тестирования Web Cache Deception${NC}"
    echo -e "${CYAN}═══════════════════════════════════════════════════════════════${NC}"
    echo ""
...
single_shot() {
    ...
    if [[ -z "$delim" ]]; then
        ...
        echo -e "${YELLOW}[*] Пример: single ; .css${NC}"
        return 1
    fi
    ...
    echo -e "${CYAN}│ РАЗДЕЛИТЕЛЬ: ${WHITE}$delim${NC}"
    echo -e "${CYAN}│ РАСШИРЕНИЕ: ${WHITE}$ext${NC}"
    echo -e "${CYAN}└─────────────────────────────────────────────────────────────┘${NC}"
    echo ""
...
quick_shot() {
    ...
    if [[ -z "$delim" ]]; then
        echo -e "${RED}[!] Укажи разделитель${NC}"
        echo -e "${YELLOW}[*] Пример: quick ; .css${NC}"
        return 1
    fi
